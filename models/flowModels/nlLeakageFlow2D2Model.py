...
from properties.dimensionlessNumbers import ReynoldsNumber

class nlLeakageFlow2D2(flowModel, ABC):
    def __repr__(self):
        return 'leakageFlow2D2Model '
    ...
    def constants(self):
        for i, region in enumerate(self.regions):
            ...
            if self.Rd[i].value < 10000000:  # Laminar
                if self.Rd[i].value <= 1E-0: # Set the value to zero to avoid indetermination
                    self.f0[i] = 0
                    self.xix[i] = 6.0 / 5.0
                    # self.eta = -0
                else:
