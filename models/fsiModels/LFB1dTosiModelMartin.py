...
# Corrected the position of G and changed the sign of Tb
...
class LFB1dTosi(object):
    ...
    def assemble(self, beam, fluid):
        ...
        S[2*esize, 2*esize] = self.Gb - np.dot(self.Bb, np.dot(Mi, self.Tb))
        S[2*esize, 2*esize+1] = np.dot(self.Bb, np.dot(Mi, self.Tt))
        ...
        S[2*esize+1, 2*esize] = np.dot(self.Bt, np.dot(Mi, self.Tb))
        S[2*esize+1, 2*esize+1] = self.Gt - np.dot(self.Bt, np.dot(Mi, self.Tt))
