...
class leakageFlow2D(fluidModel):
    ...
    def __init__(self, control, mesh, boundary, dict, name='NN'):
        ...
        inletVelocity =  self._qx0/ self._area

        # Init the base class
        self._Re = Reynolds(hydraulicDiameter, inletVelocity, dict['fluid']['type'], dict['units'])
        ...
        for key, val in self._regions.items(): #val is the boundary object
            val['name'] = key
            val['he']  = boundary[val['topBoundary']].ybot() - boundary[val['botBoundary']].ytop()
            val['he2'] = val['he']**2
            val['he3'] = val['he']**3
            val['he4'] = val['he']**4
        ...
        ReTol = 1E-10
        if self._Re.value() < 1000:
            self._ff = 48 * (1.0/(self._Re.value() + ReTol))
            self._xix = 6.0 / 5
            self._eta = 48.0 * self._nu * np.log(self._qx0 + ReTol)
        else:
            self._ff  = 0.26 * (self._Re.value() + ReTol)**(-0.24) # Friction factor
            self._xix = 1.0
    ...
    def K(self):
        ...
        for r in self._regions.values(): # Loop for filling the mass matrix
            ...
            for i in range(0, self._size):
                ...
                tvf= - gi / r['he4']
                kvf[i] += 3 * self._ff / 4 * self.integrate(tvf, r)
    ...
    def C(self):
        ...
        for r in self._regions.values(): # Loop for filling the damping matrix
            ...
            for i in range(0, self._size):
                ...
                tvf = - gix / r['he3']
                cvf[i] += (0.5 * self._ff + 0.25 * self._qx0 * self._eta) * self.integrate(tvf, r)
    ...
    def Tf(self):
        ...
        for r in self._regions.values(): # Loop for filling the damping matrix
            ...
            tfv = -(0.5 * self._ff + 0.25 * self._qx0 * self._eta) * self.integrate(1 / r['he3'], r)
            # Inlet-outlet Term
    ...
    def Dq(self):
        ...
        for r in self._regions.values(): # Loop for filling the damping matrix
            ...
            for i in range(0, self._size):
                ...
                # Friction term
                dvf += -(0.5 * self._ff + 0.25 * self._qx0 * self._eta) * si.simps(r['eigen'].ix()[i] / r['he3'], self._mesh.x())
                # Boundary Terms
    ...
    def Eq(self):
        ...
        for r in self._regions.values(): # Loop for filling the damping matrix
            ...
            for i in range(0, self._size):
                ...
                evf += -3.0 * self._ff / 4 * si.simps(gi / r['he4'], self._mesh.x())
                # Boundary Term
    ...
    def Gq(self):
        ...
        for r in self._regions.values(): # Loop for filling the damping matrix
            ...
            for i in range(0, self._size):
                ...
                # Friction Term
                Gq[r['name']] += -(0.5 * self._ff + 0.25 * self._qx0 * self._eta) * si.simps(1 / r['he3'], self._mesh.x())
                # Boundary Term
    ...
    def xxqx0(self, region): # 2.59 Tosi
        ...
        vfLoss = 0.25 * self._ff * si.simps(1 / he**3, self._mesh.x())
